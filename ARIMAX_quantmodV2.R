...
model <- auto.arima(train, xreg = pib.xreg$pib)# Create the Model
fc <- forecast(model, xreg = pib.xreg$pib, h=n)                # Forecast n periods of the data

autoplot(fc, main = "ARIMAX Forecast ONEQ 2009-2018 with exogenous variable USA GDP") + 
...
model2 <- auto.arima(train2, xreg = pib.xreg2$pib)# Create the Model
fc2 <- forecast(model2, xreg = pib.xreg2$pib, h=n2)                # Forecast n periods of the data
...
model3 <- auto.arima(train3, xreg = pib.xreg3$pib)# Create the Model
fc3 <- forecast(model3, xreg = pib.xreg3$pib, h=n3)                # Forecast n periods of the data
...
#2009-2018
me_low3 <- accuracy(fc2, test2)["Test set", "ME"]
mpe_low3 <- accuracy(fc2, test2)["Test set", "MPE"]
rmse_low3 <- accuracy(fc2, test2)["Test set", "RMSE"]


results <- data.frame(Test = c("2009-2018","2019-2023","2003-2023"), 
                      ME = c(me_low, me_low2, me_low3), 
                      MPE = c(mpe_low, mpe_low2, me_low3),
                      RMSE = c(rmse_low, rmse_low2, me_low3))
